...
def solve_vrp(
    ...
    use_google_maps: bool = False,         # real road distances instead of haversine
) -> Dict[str, Any]:
    ...
    dist_km = time_min = None
    if use_google_maps:
        # Imported lazily: google_maps_client imports back from this module.
        try:
            if not os.getenv("GOOGLE_MAPS_API_KEY"):
                raise RuntimeError("GOOGLE_MAPS_API_KEY is not set")
            from .google_maps_client import create_google_maps_distance_matrix
            dist_km, time_min = create_google_maps_distance_matrix(all_nodes, speed_kmph)
            matrix_source = "google_maps"
        except Exception as e:
            print(f"Google Maps matrix unavailable ({e}); falling back to haversine")
            dist_km = time_min = None
    ...
    if allow_drop:
        for node in range(1, n_nodes):
            ...
            # Harder-to-reach stops get a HIGHER drop penalty, so the solver keeps
            # them in the plan rather than skipping them. The weight is a fraction
            # of base_penalty per point of missing accessibility, so it scales with
            # whatever drop_penalty_per_priority the preset chose. (Weighting the
            # raw 0-100 gap instead truncates to zero: int(0.002 * 100) == 0.)
            if use_access_scores and "access_score" in stop:
                access_score = max(0, min(100, int(stop.get("access_score", 50))))
                access_penalty = int(access_penalty_weight * base_penalty * (100 - access_score))
    ...
    # Search params (must be built unconditionally - previously nested under
    # `if allow_drop:`, so allow_drop=False reached SolveWithParameters(params)
    # with params unbound)
