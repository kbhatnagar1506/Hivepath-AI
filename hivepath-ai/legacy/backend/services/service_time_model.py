...
# Resolve artifacts against the repo root, not the process CWD - running the API
# from backend/ used to miss mlartifacts/ entirely and silently fall back.
_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
ART = os.path.join(_ROOT, "mlartifacts", "service_time_gnn.pt")
ART_MLP = os.path.join(_ROOT, "mlartifacts", "service_time_mlp.pt")
...
def _normalized_access(stop: Dict[str, Any]) -> float:
    """access_score is 0-100 in the enricher and solver, but 0-1 in the models."""
    raw = stop.get("access_score")
    if raw is None:
        return 0.6
    val = float(raw)
...
def predict_minutes(stops: List[Dict[str, Any]]) -> Dict[str, float]:
    """Predict per-stop service time, keyed by stop id.

    The predictor returns a positional list; callers want a mapping, so zip it
    back onto the ids here.
    """
    if not stops:
        return {}
    ...
    rows = [{
        ...
        "access_score": _normalized_access(s),
        "hour": s.get("hour", now.hour),
        "weekday": s.get("weekday", now.weekday()),
        "node_idx": s.get("node_idx", 0),
    } for s in stops]

    minutes = predictor_singleton.predict_minutes(rows)
    return {r["id"]: float(m) for r, m in zip(rows, minutes) if r["id"] is not None}