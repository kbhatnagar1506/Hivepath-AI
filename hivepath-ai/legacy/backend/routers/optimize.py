...
async def optimize_routes(req: RouteRequest):
    ...
    # (A) access analysis - runs first so access_score is available as a feature
    # to the service time model below
    if req.use_access_analysis:
        try:
            ...
            log.warning("access analysis failed; continuing without access scores", exc_info=True)

    # (B) service time predictions
    if req.use_service_time_model:
        try:
            ...
            log.warning("service time model failed; using default service times", exc_info=True)
    ...
    if req.use_warmstart:
        try:
            ...
            log.warning("warm start failed; solving without initial routes", exc_info=True)
            warm_routes = None
