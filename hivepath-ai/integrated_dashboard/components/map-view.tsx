...
export function MapView({ activeRun, riskShaper, locations: propLocations, isLoading: propIsLoading }: MapViewProps) {
  ...
  if (loading) {
    return (
      <div className="flex h-full items-center justify-center flex-col gap-4">
        ...
        />
        <Loader2 className="h-8 w-8 animate-spin text-primary hidden" />
        <p className="text-sm text-muted-foreground">Loading Next-Gen Infrastructure Platform...</p>
      </div>
    )
  }
  ...
  const avgRisk = stops.length > 0 ? stops.reduce((sum, s) => sum + (s.weather_risk + s.crime_risk + s.traffic_risk) / 3, 0) / stops.length : 0
  const totalDemand = stops.reduce((sum, s) => sum + s.demand, 0)
  const avgAccessScore = stops.length > 0 ? stops.reduce((sum, s) => sum + s.access_score, 0) / stops.length : 0
  const highRiskStops = stops.filter(s => (s.weather_risk + s.crime_risk + s.traffic_risk) / 3 > 0.7).length
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-start justify-between gap-6">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                />
                <Route className="h-8 w-8 text-primary hidden" />
                <div>
                  <h2 className="text-2xl font-bold text-foreground">🏗️ Smart Infrastructure Network</h2>
                  <p className="text-sm text-muted-foreground">
                    Reimagining transportation with smarter grids, stronger infrastructure, and faster logistics
                  </p>
                </div>
              </div>
            ...
            <div className="flex gap-3">
              <Card className="p-4 bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border-emerald-500/20 min-w-[140px]">
                ...
                <div className="text-[10px] text-muted-foreground mt-1">Infrastructure resilience</div>
              </Card>
              ...
              <Card className="p-4 bg-gradient-to-br from-blue-500/10 to-blue-500/5 border-blue-500/20 min-w-[140px]">
                ...
                <div className="text-[10px] text-muted-foreground mt-1">vs traditional systems</div>
              </Card>
            </div>
          </div>
          ...
          <div className="mt-4 p-3 rounded-lg bg-card/50 border border-border/50">
            <p className="text-sm text-muted-foreground leading-relaxed">
              <span className="font-semibold text-foreground">Next-Gen Infrastructure:</span> This platform reimagines transportation 
              with AI-powered self-healing systems that adapt to real-world conditions. Our technology fuels tomorrow's 
              smarter grids, stronger infrastructure, and faster logistics networks.
              {riskShaper && showRiskHeat && (
                <span className="text-primary font-medium"> The heatmap shows intelligent risk avoidance zones.</span>
              )}{" "}
              Click any stop marker to see our advanced infrastructure intelligence.
            </p>
          </div>
        </div>
      </div>
                      ...
                      >
                        ...
                        {stop.demand > 5 && (
                          <div className="absolute -bottom-1 -right-1 h-4 w-4 rounded-full bg-blue-500 border-2 border-background flex items-center justify-center">
                            <span className="text-[8px] font-bold text-white">{stop.demand}</span>
                          </div>
                        )}
                        
                        {/* Access score indicator */}
                        <div className="absolute -top-1 -left-1 h-4 w-4 rounded-full border-2 border-background flex items-center justify-center"
                             style={{
                               backgroundColor: stop.access_score > 0.7 ? '#10b981' : stop.access_score > 0.4 ? '#f59e0b' : '#ef4444'
                             }}>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-2 mt-2">
                    <Badge variant="secondary" className="text-xs">
                      Priority: {selectedStop.priority}
                    </Badge>
                    <Badge variant="outline" className="text-xs">
                      Demand: {selectedStop.demand}
                    </Badge>
                  </div>
                </div>
                <Button
